#!/usr/bin/env python
# CoinMarketCap IRC Bot
# Developed by acidvegas in Python
# https://git.acid.vegas/cmc
# cmcirc.py
		...
		try:
			if msg[:1] in '!@$':
				if time.time() - self.last < throttle_cmd:
					...
					args = msg.split()
					if msg == '@cmc':
						self.sendmsg(chan, bold + 'CoinMarketCap IRC Bot - Developed by acidvegas in Python - https://git.acid.vegas/cmc')
